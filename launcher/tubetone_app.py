...
def run_gui() -> None:
    ...
    try:
        ...
        w = min(1040, max(780, sw - 80))
        h = min(920, max(640, int(sh * 0.86)))
    ...
    def make_card(parent, padding: int = 10, **pack):
        return make_glass_card(parent, padding=padding, elevate=1, **pack)
    ...
    shell_inner.pack(fill="both", expand=True, padx=14, pady=10)

    # Compact header (no version badge — keeps vertical space free)
    header_panel, header = make_glass_card(shell_inner, padding=8, elevate=1)
    ...
    tip_var = tk.StringVar(
        value="Paste links → pick Music (MP3) or Video (MP4) in Quality → hit Download"
    )

    # Scrollable body: tip + tabs + progress/actions — scrollbar when window is short
    body_scroll = ScrollFrame(shell_inner)
    body_scroll.pack(fill="both", expand=True)
    body = body_scroll.interior
    ...
    tip_lbl.pack(fill="x", pady=(2, 4))

    notebook_host = tk.Frame(body, bg=G["bg_deep"])
    notebook_host.pack(fill="x", expand=False)
    ...
    # Progress + primary actions (scroll with body so they never clip off permanently)
    footer = tk.Frame(body, bg=G["bg_deep"])
    ...
    dl_scroll_host.pack(fill="x", expand=False, padx=2, pady=(2, 0))

    # Links: compact height; rest of page scrolls if the window is short
    _, card_inner = make_card(dl_scroll_host, fill="x", expand=False)
    section_label(
        card_inner,
        "Add links",
    ...
    ttk.Label(card_inner, textvariable=info_var, style="GlassMuted.TLabel").pack(anchor="w", pady=(2, 0))
    ...
    _, mode_row = make_card(dl_scroll_host, fill="x", expand=False, pady=(6, 2))
    section_label(
        mode_row,
        "Quality & save folder",
    ...
    fmt_row = ttk.Frame(mode_row, style="Glass.TFrame")
    fmt_row.pack(fill="x", pady=(6, 4))
    ttk.Label(fmt_row, text="FORMAT", style="GlassMuted.TLabel").pack(anchor="w")
    mode_seg = SegmentBar(
        fmt_row,
        [("music", "Music · MP3"), ("video", "Video · MP4")],
    ...
    fmt_menu.grid(row=1, column=2, sticky="w", padx=(12, 0), pady=(2, 0))
    ...
    ).grid(row=1, column=3, sticky="w", padx=(12, 0), pady=(2, 0))
    ttk.Checkbutton(
        extra, text="Thumbnail", variable=thumb_var, style="Dark.TCheckbutton"
    ).grid(row=1, column=4, sticky="w", padx=(8, 0), pady=(2, 0))
    ...
    def layout_options() -> None:
        ...
        if is_video:
            ...
            quality_menu.grid(row=1, column=col, sticky="w", pady=(2, 0))
            col += 1
        bitrate_label.configure(text="AUDIO BITRATE" if is_video else "BITRATE")
        bitrate_label.grid(row=0, column=col, sticky="w", padx=(12 if col else 0, 0))
        bitrate_menu.grid(
            row=1, column=col, sticky="w", padx=(12 if col else 0, 0), pady=(2, 0)
        )
        col += 1
        folder_label.grid(row=0, column=col, sticky="w", padx=(12, 0))
        out_entry.grid(
            row=1, column=col, sticky="we", padx=(12, 6), pady=(2, 0), ipady=3
        )
        opts.columnconfigure(col, weight=1)
        browse_btn.grid(row=1, column=col + 1, sticky="e", pady=(2, 0))
    ...
    # Progress + actions — compact: title + status + thin bar (no empty song row)
    _, prog = make_card(footer, padding=8, fill="x", pady=(4, 4))
    prog_head = ttk.Frame(prog, style="Glass.TFrame")
    prog_head.pack(fill="x")
    ttk.Label(prog_head, text="Progress", style="GlassTitle.TLabel").pack(side="left")
    pct_var = tk.StringVar(value="0%")
    ttk.Label(prog_head, textvariable=pct_var, style="GlassPct.TLabel").pack(side="right")
    detail_var = tk.StringVar(value="Ready — paste a link and hit Download")
    ttk.Label(prog, textvariable=detail_var, style="GlassMuted.TLabel").pack(
        anchor="w", pady=(2, 0)
    )
    ...
    # packed only when a title/URL is present (see set_progress)
    ...
    def set_progress(pct: float, detail: str, song: str = "") -> None:
        ...
        def apply() -> None:
            ...
            if song:
                song_var.set(song)
                try:
                    if not song_lbl.winfo_ismapped():
                        song_lbl.pack(anchor="w", pady=(2, 0))
                except tk.TclError:
                    pass
            try:
                body_scroll.refresh()
    ...
    def show_about() -> None:
        messagebox.showinfo(
            "About YTMP",
            "YTMP — download & convert media locally\n\n"
            "Tabs: Download · Convert · Library\n\n"
            f"Data: {DATA_DIR}\n"
    ...
    # Primary actions
    primary_row = tk.Frame(btns, bg=G["bg_deep"])
    primary_row.pack(fill="x")
    secondary_row = tk.Frame(btns, bg=G["bg_deep"])
    secondary_row.pack(fill="x", pady=(4, 0))

    download_btn = GlassButton(primary_row, text="Download MP3", command=do_download, primary=True)
    download_btn.pack(side="left", padx=(0, 10))
    cancel_btn = GlassButton(secondary_row, text="Cancel", command=cancel_download)
    cancel_btn.pack(side="left", padx=(0, 8))
    cancel_btn.configure(state="disabled")
    clear_btn = GlassButton(secondary_row, text="Clear", command=clear_urls)
    clear_btn.pack(side="left", padx=(0, 8))
    GlassButton(secondary_row, text="Open folder", command=open_out).pack(side="left", padx=(0, 8))
    GlassButton(secondary_row, text="Play last", command=play_last).pack(side="left")
    ...
    conv_host.pack(fill="x", expand=False, padx=2, pady=2)
    _, conv_inner = make_card(conv_host, fill="x", expand=False)
    section_label(
        conv_inner,
        "Convert a file",
        "Local audio/video → MP3, WAV, FLAC, M4A, Opus, or MP4",
        step="1",
    )
    ...
    lib_host.pack(fill="x", expand=False, padx=2, pady=2)
    _, lib_inner = make_card(lib_host, fill="x", expand=False)
    section_label(lib_inner, "Library", "Search past downloads and exports")
    lib_q = tk.StringVar()
    lib_bar = ttk.Frame(lib_inner, style="Glass.TFrame")
    lib_bar.pack(fill="x", pady=(6, 4))
    ...
    def _refresh_body_scroll(_e=None) -> None:
        try:
            w = max(body_scroll._canvas.winfo_width() - 16, 200)
            tip_lbl.configure(wraplength=w)
            body_scroll.refresh()
        except Exception:
            pass

    body_scroll._canvas.bind("<Configure>", _refresh_body_scroll, add="+")
    root.bind("<Configure>", lambda e: root.after_idle(_refresh_body_scroll) if e.widget is root else None)
    root.after(80, _refresh_body_scroll)
    root.after(400, _refresh_body_scroll)
